	...
	public void map(LongWritable ikey, Text ivalue, Context context) throws IOException, InterruptedException {
		try {
			...
			if( m_y.contains(dt) || dt.equals("all")) {
				String shop = arr[0];
				long amount = Long.parseLong(arr[5].split("\\.")[0]);
				long price = Long.parseLong(arr[6].split("\\.")[0]);
				long discount = Long.parseLong(arr[7].split("\\.")[0]);
				long total = price*amount - discount;
				context.write(new Text(shop), new LongWritable(total));
			}
		}
		catch(Exception e) {
